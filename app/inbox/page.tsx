...
export default function InboxPage() {
  ...
  async function capture(e:FormEvent){e.preventDefault();const text=value.trim();if(!text)return;const response=await fetch("/api/db",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"inbox",text})});const item=await response.json();if(!response.ok)throw new Error(item.error||"Capture failed.");setItems(v=>[item,...v]);setValue("");}
  async function organize(id:string){const response=await fetch("/api/db",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"inbox",id,status:"processed"}));const item=await response.json();if(!response.ok)throw new Error(item.error||"Organization failed.");setItems(v=>v.map(x=>x.id===item.id?item:x));}
