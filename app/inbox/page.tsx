...
export default function InboxPage() {
  ...
  async function organize(id:string){const item=await fetch("/api/db",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({type:"inbox",id,status:"organized"})}).then(r=>r.json());setItems(v=>v.map(x=>x.id===item.id?item:x));}
