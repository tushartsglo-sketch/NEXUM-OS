...
export default function ProjectsPage(){
 ...
 async function advance(p:Project){const progress=Math.min(100,p.progress+10);const status=progress===100?"complete":"active";const updated=await fetch("/api/projects",{method:"PATCH",headers:{"Content-Type":"application/json"},body:JSON.stringify({id:p.id,progress,status})}).then(r=>r.json());setProjects(v=>v.map(x=>x.id===updated.id?updated:x));}
