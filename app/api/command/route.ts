...
export async function POST(request: NextRequest) {
  try {
    ...
    if (confirm && body.task && typeof body.task === "object" && typeof body.task.title !== "undefined") {
      const task = body.task as { date?: string; time?: string; recurrence?: string; recurrenceRule?: string; priority?: string; reminderMinutes?: number };
      ...
      const recurrenceRule = typeof task.recurrenceRule === "string" ? task.recurrenceRule : null;
    ...
    if (interpreted && interpreted.confidence >= 0.85) {
      if (interpreted.intent === "task") {
        return NextResponse.json({
          ...
          task: { title: interpreted.text, date: interpreted.date, time: interpreted.time, recurrence: interpreted.recurrence, recurrenceRule: interpreted.recurrenceRule, priority: interpreted.priority, reminderMinutes: interpreted.reminderMinutes }
        });
      }
      if (interpreted.intent === "note") {
        return NextResponse.json({ type: "confirmation_required", kind: "knowledge", title: interpreted.text, command, item: { title: interpreted.text.slice(0, 80), type: "note", topic: "General", content: interpreted.text, tags: "" } });
      }
      if (interpreted.intent === "research") {
        return NextResponse.json({ type: "confirmation_required", kind: "research", title: interpreted.text, command, item: { title: interpreted.text.slice(0, 100), question: interpreted.text, status: "idea" } });
      }
    }
    ...
    if (recurringMatch) {
      ...
      const title=recurringMatch[1].replace(/\s+(?:remind(?:er)?\s+(?:me\s+)?(?:at\s+the\s+exact\s+time|exactly|0\s+minutes?|30\s+minutes?|1\s+hour|60\s+minutes?)\s+before)$/i,"").trim();
      const reminderText=recurringMatch[1].match(/(at\s+the\s+exact\s+time|exactly|0\s+minutes?|30\s+minutes?|1\s+hour|60\s+minutes?)\s+before/i)?.[1]?.toLowerCase()||"";
      const reminderMinutes=reminderText.includes("30")?30:(reminderText.includes("1 hour")||reminderText.includes("60")?60:0);
      const task=await db.task.create({data:{title,date,time:date.toTimeString().slice(0,5),duration:30,status:"todo",priority:"medium",recurrence,reminderMinutes}});
      return NextResponse.json({type:"created",kind:"task",item:task,parser:"recurring-task"});
    }
    if (relativeTask) {
      ...
      const reminderText=command.match(/(30\s+minutes?|1\s+hour|60\s+minutes?|exact(?:ly)?|at\s+the\s+exact\s+time)\s+before/i)?.[1]?.toLowerCase()||"";
      const reminderMinutes=reminderText.includes("30")?30:(reminderText.includes("1 hour")||reminderText.includes("60")?60:0);
      const task=await db.task.create({data:{title:relativeTask[1].trim(),date,time:date.toTimeString().slice(0,5),duration:30,status:"todo",priority:"medium",reminderMinutes}});
