...
export async function POST(request: NextRequest) {
  ...
  if (type === "knowledge") { const created = await db.knowledgeEntry.create({ data: { title: body.title, type: body.entryType || "note", topic: body.topic || "General", tags: body.tags || "", content: body.content || "", embedding: await createEmbedding(body.title + "\n" + (body.topic || "") + "\n" + (body.tags || "") + "\n" + (body.content || "")) } }); return NextResponse.json(created); }
  ...
  if (type === "task") return NextResponse.json(await db.task.create({ data: { title: body.title, description: body.description || null, date: new Date(body.date), time: body.time || "09:00", duration: Number(body.duration || 30), priority: body.priority || "medium", project: body.project || null, recurrence: body.recurrence || "none" } }));
  return NextResponse.json({ error: "Unknown data type." }, { status: 400 });
}
