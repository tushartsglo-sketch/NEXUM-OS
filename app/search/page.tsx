...
export default function SearchPage(){
 const [q,setQ]=useState(""),[semantic,setSemantic]=useState(true),[results,setResults]=useState<Result[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState("");
 useEffect(()=>{const t=setTimeout(async()=>{if(!q.trim()){setResults([]);setError("");return}setLoading(true);setError("");try{const r=await fetch("/api/search?q="+encodeURIComponent(q)+(semantic?"&semantic=1":""));const d=await r.json();if(!r.ok)throw new Error(d.error||"Search failed.");setResults(d.results||[])}catch(e){setResults([]);setError(e instanceof Error?e.message:"Search failed.")}finally{setLoading(false)}},300);return()=>clearTimeout(t)},[q,semantic]);
 ...
 {loading&&<div className="empty-state">Searching the archive…</div>}{error&&<div className="empty-state">{error}</div>}
