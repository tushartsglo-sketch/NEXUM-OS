...
export default function Home() {
 ...
 return <main className="shell">
  <aside className="sidebar"><div className="brand">NEXUM</div><nav>{nav.map(([x,href],i)=><Link className={i===0?"nav active":"nav"} href={href} key={x}>{x}</Link>)}</nav><div className="sideFoot">PRIVATE WORKSPACE<br/><span>Personal archive</span></div></aside>
  ...
  <div className="panel"><div className="head"><div><p className="eyebrow">KNOWLEDGE</p><h2>Your archive</h2></div><Link className="link" href="/knowledge">Open library</Link></div><div className="stats">{library.map(x=><div className="stat" key={x[1]}><b>{x[0]}</b><span>{x[1]}</span></div>)}</div><div className="continue"><em>CONTINUE</em><strong>Consumer Psychology</strong><p>7 connected notes</p></div></div></section>
  <section className="grid lower"><div className="panel capture"><p className="eyebrow">QUICK CAPTURE</p><h2>Put it in NEXUM.</h2><p>Save a thought, source, link, question, or idea before you lose it.</p><Link className="primary" href="/inbox">Open inbox</Link></div><div className="panel"><p className="eyebrow">RECENT</p><h2>Research threads</h2><div className="thread"><b>01</b><div><strong>Behavioral economics</strong><p>12 notes · 4 sources · 2 open questions</p></div></div><div className="thread"><b>02</b><div><strong>Digital products</strong><p>8 notes · 6 sources · 3 ideas</p></div></div></div></section>
  </section></main>;
}
